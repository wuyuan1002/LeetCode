...
//   - 当前行不是最后一行，且不只一个单词：
//     需要将空格均匀分配在单词之间，首先计算好平均每个单词之间需要放多少个空格，然后再将余数的空格均匀分给前面的间隙
//     即每个单词之间应至少有 avgSpaces = numSpaces / (numWords-1) 个空格，
...
func fullJustify(words []string, maxWidth int) []string {
	...
	for {
		// 更新当前行的第一个单词下标为前一行的最后一个单词下标后一位
		left = right

		// 计算当前行可以放多少单词 -- 注意单词之间应至少有一个空格
