...
// 并查集：用于解决一些元素分组的问题，用来管理一系列不相交的集合，即图中互相没有边连接的孤立节点区域，也就是图中互相没有被连通的节点区域（即连通分量）
...
func findRedundantConnection(edges [][]int) []int {
	// 定义一个数组表示并查集，用来存储每个节点的父节点是哪个 -- parent[a]=b 表示节点a的父节点是b，即节点a存在一条指向节点b的边
	...
	for i := range edges {
		...
		// 尝试将当前两节点连接，若不能连接则说明连接当前两节点导致出现了环，为附加的边
		if !connect(parent, from, to) {
			return []int{from, to}
		}
	}
...
// connect 给定两个节点尝试将其进行连接，若两节点已处于同一个连通分量中则说明连接这两节点会出现环，返回false，若两节点分别在不同的连通分量中，则将两节点进行相连，返回true
func connect(parent []int, from, to int) bool {
	// 分别获取两节点所在连通分量的跟节点
	fromRoot, toRoot := getRoot(parent, from), getRoot(parent, to)
...
// getRoot 给定一个节点，获取该节点在该连通分量中的跟节点
