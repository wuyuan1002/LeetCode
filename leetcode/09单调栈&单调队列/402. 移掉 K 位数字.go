...
//
// 使用单调递减栈，栈顶元素为栈内最大值，当新入栈的元素大于栈顶元素时，说明栈内高位的数字比其右侧数字大了，
// 此时不断从栈顶开始将大于当前元素的高位数字出栈，这些数字就是要被删掉的数字，直到新的栈顶元素不大于当前数字或栈为空或已经删除够k个数字
func removeKdigits(num string, k int) string {
	// 单调递减栈 -- 从栈顶到栈底单调递减，栈内为当前高位数字能构成的最小值
	stack := make([]byte, 0)
	...
	// 为什么是从栈顶删呢？因为栈顶到栈底是单调递减的，越靠近栈顶的元素越大，所以需要将大的数字删掉"12345" -> "123"，而不是"12345" -> "345"
