...
// ---------
//
// 单调递增栈：栈底到栈顶是单调递增的，栈顶元素为栈内最大值
// 单调递减栈：栈底到栈顶是单调递减的，栈顶元素为栈内最大值
//
// 什么时候用单调栈什么时候用单调队列呢？
// 求队列的最大最小值时用单调队列：Offer 59-2、leetcode 239. 滑动窗口最大值
// 求栈的最大最小值时用单调栈：leetcode 155. 最小栈
//
// 求最大值时用单调递增，求最小值时用单调递减
// 单调递增栈：从栈底到栈顶是递增的、单调递增队列：从队头到队尾是递增的
// eg：
// 单调递减队列入队时：从单调队列队尾入队，若队尾元素小于被添加元素则将其删掉，直到将被添加元素加入到队尾
// 单调递减队列出队时：若出队元素正好是最大值，则同步从单调队列中出队，即更新队列最大值
// 获取队列最大值：单调递减队列队头元素为当前队列最大值
//
// 单调递减栈入栈时：若添加的元素小于等于单调栈的栈顶元素，则同步入栈到单调栈中，即更新栈内的最小值
// 单调递减栈出栈时：若弹出的元素正好是最小值，则同步从单调栈中弹出，即更新栈内最小值
// 获取栈的最小值：单调递减栈的栈顶元素为当前栈的最小值
// ---------
