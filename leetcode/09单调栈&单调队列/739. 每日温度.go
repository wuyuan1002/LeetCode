...
//
// 本题其实就是求数组中每个元素和其下一个更大元素的距离
func dailyTemperatures(temperatures []int) []int {

	// 总结果
	result := make([]int, len(temperatures))
	// 单调递减栈，用于存还没有出现比自己更高温度的天，栈内元素都是待计算的，温度低的位于栈顶
	// 因为要求的是每个元素和他下一个更大元素的距离，所以栈内存的是元素下标，使用下标计算两元素的距离
	stack := make([]int, 0)

	for i := 0; i < len(temperatures); i++ {
		// 若当前元素大于单调递减栈顶元素，说明当前元素是栈内部分元素的下一个比自己大的数字，栈内部分元素找到了比自己更高温度出现的天
