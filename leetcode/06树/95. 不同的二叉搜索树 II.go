...
// generateTrees .
// leetcode 96. 不同的二叉搜索树 -- 96题只需返回可能的情况个数，本题要返回所有可能的解
//
// 本题不能使用动态规划，动态规划只能得出解的个数而得不到解具体是什么，因此需要使用递归真正生成每棵树
// 从 [1, n] 中挨个选择每个节点 i 作为树的根节点，然后分别递归求 [1, i) 和 (i, n] 的所有不同二叉树，
// 它们求出的就是当前节点 i 的左右子树的所有可能，然后从其中获取每个子树拼接到当前树上即得到一个解
...
func generate(start, end int) []*TreeNode {
	...
	// 遍历[start, end] -- 生成选择每一个节点i作为根节点的所有不同二叉树
	for i := start; i <= end; i++ {
		// 分别求解左右子树的所有解
