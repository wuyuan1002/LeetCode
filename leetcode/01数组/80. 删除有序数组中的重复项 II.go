...
// removeDuplicates80 .
// 同 leetcode 26. 删除有序数组中的重复项
// 双指针 -- 左指针左面的数字都是满足条件的（不包括左指针本身指向的数字）
...
func removeDuplicates80(nums []int) int {
	...
	// 向前遍历右指针，寻找与左指针前2位不一样的数字交换给左指针然后向前移动左指针
	for r < len(nums) {
		// 若当前右指针指向的数字与左指针左侧前2位数字不一致，说明这个数字出现没超过2次，将其赋值给左指针所指位置并将左指针向前移动一位
		if nums[r] != nums[l-2] {
			nums[l] = nums[r]
			l++
		}
