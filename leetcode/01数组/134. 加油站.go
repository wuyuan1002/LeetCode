...
//
// 每个加油站对车辆的贡献值为 gas[i] - cost[i]
func canCompleteCircuit(gas []int, cost []int) int {
	// 表示从start出发能够绕行一周
	start := 0

	// 从start加油站出发时油箱当前的剩余油量、所有加油站的贡献值
	currentRemain, totalRemain := 0, 0

	// 遍历以第i个加油站作为起点，查看能否绕行一周
	for i := 0; i < len(gas); i++ {
		// 计算到达当前加油站，当前加油站能够为汽车贡献的油量
		currentRemain += gas[i] - cost[i]
		totalRemain += gas[i] - cost[i]

		// 若当前油箱剩余油量不足，说明不足以到达当前加油站，那么继续计算以下一个加油站出发能否绕行一周
		if currentRemain < 0 {
			start = i + 1
			currentRemain = 0
		}
	...
	// totalRemain表示所有加油站的贡献值，若小于0说明所有加油站的油量总和小于消耗的油量，一定不能绕行一周
	if totalRemain < 0 {
		return -1
	}
