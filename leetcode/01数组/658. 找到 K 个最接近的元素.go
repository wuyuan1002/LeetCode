...
//
// 将数组按照「更靠近」进行重新排序，排序后数组的前k个数字就是答案
// 由于数组本身就是从小到大排序好的，所以若差值相同时，本身在前的更靠近，
// 因此使用稳定排序即可在差值相同时不破坏原有顺序
func findClosestElements1(arr []int, k int, x int) []int {
	// 稳定排序，在差值绝对值相同的情况下，保证更小的数排在前面（因为原数组本身已经是排好序的）
	sort.SliceStable(arr, func(i, j int) bool { return abs(arr[i]-x) < abs(arr[j]-x) })

	// 数组前k个数数字就是最靠近x的k个数字，对这k个数字进行排序后返回
	arr = arr[:k]
	sort.Ints(arr)
	return arr
}
...
// 若差值不同，则差值越小越靠近，若差值相同则数字越小越靠近
//
// 先使用二分查找找到数组中最接近x的数字，然后使用双指针向两边进行拓展，最终的区间为(left, right)
