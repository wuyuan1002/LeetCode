...
// 也就是求数组中能够将出现次数最多的数字全部包含的子数组最短长度
//
// 先求出数组中出现次数最多的数字（可能有多个），以及其首次和最后一次出现的下标位置，
// 然后对于出现次数最多的数字，比较其首次出现和最后一次出现的下标差，取最小值即为答案
func findShortestSubArray(nums []int) int {
	// 定义一个map，用来存每个数字出现的次数、首次出现的下标、最后一次出现的下标
	type entry struct{ count, left, right int }
	hash := make(map[int]entry)

	// 遍历数组，统计每个数字的信息 -- 每个数字的出现次数、首次出现位置、最后一次出现位置
	for i, n := range nums {
